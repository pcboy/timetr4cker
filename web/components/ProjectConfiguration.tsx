...
export const ProjectConfiguration = observer(({ projectName }) => {
  ...
  const projectRate = (
    <RIENumber
      ...
      change={(data) => {
        console.log(data);
        entryStore.updateProject({ rate: data.rate });
      }}
    ></RIENumber>
  );
  ...
  const budget = (
    <RIENumber
      ...
      className="editable"
      propName="budget"
      change={(data) => entryStore.updateProject({ budget: data.budget })}
    ></RIENumber>
  );
    ...
    <SProjectConfiguration className="project-configuration">
      <div className="columns is-multiline">
        ...
        <div className="column is-12">
          ...
          <span>
            {`${(entryStore.project.currentMinutes / 60.0).toFixed(
              1
            )} hours / `}
          </span>
        ...
        <div className="column is-12">
          ...
          <span>
            {(
              (entryStore.project.currentMinutes / 60.0) *
              entryStore.project.rate
            ).toFixed(0)}{" "}
